...
for i in runs:
    ...
    h_mass          =TH1F('h_mass','',100,124.9999,125.0001)
    ...
    for ievent in range(NEntries):
        ...
        for i in range(len(mom0status)):
            ...
            if pdgID==22 and mom0pid[i]==25 and momstatus==22:
...
for hist in range(len(histspt)):
    ...
    if hist==0:
        histspt[hist].SetXTitle("p_{T} of h [GeV]")
        ...
        histspt[hist].SetXTitle("pT_{h} [GeV")
        histspt[hist].SetYTitle("Events")
        histspt[hist].SetMaximum(10)
        histspt[hist].SetLineColor(hist+1)
        histspt[hist].Rebin(2)
...
texcms.Draw("same")
texCat.Draw("same")
t = ROOT.TPaveLabel(0.1, 0.96, 0.95, 0.99, "p_{T} comparison of h", "brNDC")
t.Draw('same')
c.Update()

c.SaveAs("Combined_h_pt_a0300.png")
c.SaveAs("Combined_h_pt_a0300.root")
...
for hist in range(len(histseta)):
    ...
    if hist==0:
        histseta[hist].SetXTitle("#eta_{h}")
...
texcms.Draw("same")
texCat.Draw("same")
t = ROOT.TPaveLabel(0.1, 0.96, 0.95, 0.99, "#eta_{h} comparison", "brNDC")
t.Draw('same')
c.Update()

c.SaveAs("Combined_h_eta_a0300.png")
c.SaveAs("Combined_h_eta_a0300.root")
...
for hist in range(len(histsphi)):
    #legend = CreateLegend(0.70, 0.94, 0.75, 0.92, "M_{A0}=300 GeV")
    if hist==0:
        histsphi[hist].SetXTitle("#phi_{h}")
...
texcms.Draw("same")
texCat.Draw("same")
t = ROOT.TPaveLabel(0.1, 0.96, 0.95, 0.99, "#phi_{h} comparison", "brNDC")
t.Draw('same')
c.Update()

c.SaveAs("Combined_h_phi_a0300.png")
c.SaveAs("Combined_h_phi_a0300.root")
...
for hist in range(len(histsmass)):
    #legend = CreateLegend(0.70, 0.94, 0.75, 0.92, "M_{A0}=300 GeV")
    if hist==0:
        histsmass[hist].SetXTitle("m_{#gamma#gamma} [GeV]")
        histsmass[hist].SetYTitle("Events")
        histsmass[hist].GetXaxis().SetNdivisions(508)
...
texcms.Draw("same")
texCat.Draw("same")
t = ROOT.TPaveLabel(0.1, 0.96, 0.95, 0.99, "m_{h} comparison", "brNDC")
t.Draw('same')
c.Update()

c.SaveAs("Combined_h_mass_a0300.png")
c.SaveAs("Combined_h_mass_a0300.root")
